...
import br.com.battlebits.commons.util.string.StringTimeUtils;
...
import tk.yallandev.saintmc.game.util.MapUtils;
...
public class HungerGamesMode extends GameMode {
	...
	public void startGame() {
		...
		getGameMain().getServer().getPluginManager().registerEvents(new DeathListener(getGameMain(), this),getGameMain());
		...
		scheduler.pulse(new ScheduleArgs(getGameMain().getGameType(), getGameMain().getGameStage(), getGameMain().getTimer()));
		...
		getGameMain().getServer().getPluginManager().registerEvents(new InvincibilityListener(getGameMain()), getGameMain());
...
//TODO			Member.getMember(p.getUniqueId()).removePermission("tag.winner");
	...
	public void checkWinner() {
		...
		BattlePlayer battlePlayer = BattlePlayer.getPlayer(win.getUniqueId());
//		battlePlayer.addPermission("tag.winner");
		battlePlayer.addMoney(new Random().nextInt(50));
		
		int xp = new Random().nextInt(win.getKills() == 0 ? 10 : 15 * win.getKills());
