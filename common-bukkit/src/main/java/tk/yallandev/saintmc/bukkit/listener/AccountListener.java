...
public class AccountListener implements Listener {
    ...
    public void onAsyncPlayerPreLogin(AsyncPlayerPreLoginEvent event) {
        ...
        if (Bukkit.getPlayer(uniqueId) != null) {
            ...
            event.setKickMessage("§4§lCONTA\n§f\n§fO jogador " + event.getName() + " já está online no servidor!");
            return;
        }
    ...
    public void onPlayerLogin(PlayerLoginEvent event) {
        ...
        if (CommonGeneral.getInstance().getMemberManager().getMember(event.getPlayer().getUniqueId()) == null) {
            event.disallow(PlayerLoginEvent.Result.KICK_OTHER, "§4§lCONTA\n§f\n§fNão foi carregada sua conta!");
            return;
        }
    ...
    public void onLeave(PlayerQuitEvent event) {
        Bukkit.getScheduler().runTaskAsynchronously(BukkitMain.getInstance(), () -> removePlayer(event.getPlayer().getUniqueId()));
