...
public class ShopInventory {
    ...
    public ShopInventory(Player player, int page) {
        ...
        List<Kit> kits = new ArrayList<>(
                GameMain.getInstance().getKitManager().getKitList().stream().filter(kit -> !gamer.hasKitPermission(kit))
        ...
        for (Kit kit : kits) {
            items.add(new MenuItem(new ItemBuilder()
                                           ...
                                           .type(kit.getKitType()).name(money >= kit.getPrice() ?
                                                                        "§a" + NameUtils.formatString(kit.getName()) :
                                                                        "§c" + NameUtils.formatString(kit.getName()))
                                           .build(), new BuyKit(kit, page)));
        }
        ...
        if (items.isEmpty()) {
            menu.setItem(22, new ItemBuilder().type(Material.STAINED_GLASS_PANE).durability(6)
                                              .lore("§7Você possui todos os kits do KitPvP!")
                                              .name("§aNenhum kit para comprar!").build());
        } else {
    ...
    public static class BuyKit implements MenuClickHandler {
        ...
        public boolean onClick(Player p, Inventory inv, ClickType type, ItemStack stack, int slot) {
            ...
            if (member.getMoney() >= kit.getPrice()) {
                ...
                member.sendMessage("§cVocê não tem money o suficiente para comprar o kit " +
                                   NameUtils.formatString(kit.getName()) + ", você precisa de mais " +
                                   (kit.getPrice() - member.getMoney()) + "!");
                p.closeInventory();
            }
