...
public class KitpvpInventory {
	...
	public KitpvpInventory(Player player) {
		...
		MenuInventory menu = new MenuInventory("§7§nSelecionar PvP",
	...
	public void create(List<ProxiedServer> serverList, Member member, MenuInventory menu) {
		...
		for (ProxiedServer server : serverList) {
			...
			String name = server.getServerType() == ServerType.FULLIRON ? "Fulliron" : "Simulator";

			if (server.isFull()) {
				builder.name("§e" + name + " #" + server.getServerId().substring(1, 2).toUpperCase());
				builder.durability(7);
			} else {
				builder.name("§a" + name + " #" + server.getServerId().substring(1, 2).toUpperCase());
