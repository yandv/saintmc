...
public class GameKit {
	...
	public GameKit(Player player) {
		if (InventoryListener.TESTING.contains(player.getUniqueId())) {
			player.sendMessage("§c§l> §fO kit aleatório já está sendo selecionado!");
			return;
		}
		...
		start();
		player.sendMessage("§9Aleatório> §fO kit aleatório está sendo escolhido!");
	}
			...
			public void run() {
				...
				if (x == 28) {
					...
					new BukkitRunnable() {
						...
						@Override
						public void run() {
							player.sendMessage("§a§l> §fO seu kit aleatório da partida foi o §a" + NameUtils.formatString(kit.getName()) + "§f!");
