...
//		}
		...
		if(!Gamer.getGamer(event.getPlayer()).hasKit(event.getKit().getName().toLowerCase())) {
			event.getPlayer().sendMessage("§c§l> §fVocê não possui o kit §c" + NameUtils.formatString(event.getKit().getName()));
			event.setCancelled(true);
			return;
		}
		
		event.getPlayer().sendMessage(" §a* §fVocê selecionou o kit §a" + NameUtils.formatString(event.getKit().getName()) + "§f!");
	}
}
