...
public class PackageCommand implements CommandClass {
    ...
    @Command(name = "givevip", usage = "/<command> <player> <tempo> <group>", groupToUse = Group.GERENTE, aliases = {"darvip","tempgroup"})
    public void givevip(BungeeCommandArgs cmdArgs) {
        final CommandSender sender = cmdArgs.getSender();
        final String[] args = cmdArgs.getArgs();
        
        if (args.length != 3) {
            sender.sendMessage(" §e* §fUse §a/tempgroup <player> <tempo> <group>§f para setar um grupo temporariamente.");
            return;
        }
        ...
        try {
            ...
            sender.sendMessage(" §c* §fO grupo §c" + args[2].toUpperCase() + "§f não existe ou não pode ser setado como temporario.");
            return;
        }
        ...
        sender.sendMessage(" §a* §fVocê deu o vip §a§l"  + rank.name() + "§f para o " + player.getPlayerName() + "(" + player.getUniqueId().toString().replace("-", "") + ")" + " com a dura§§o de " + DateUtils.formatDifference(expiresCheck / 1000) + "!");
//        
//        BungeeMain.getPlugin().getDiscordManager().sendMessage(new EmbedBuilder().setTitle("LonneMC - " + (cmdArgs.isPlayer() ? cmdArgs.getPlayer().getName() : "CONSOLE")).appendDescription("\nO jogador " + player.getName() + " teve seu cargo atualizado (" + rank.name() + " de " + DateUtils.formatDifference(language, expiresCheck / 1000) + ")").setColor(Color.YELLOW).build(), BungeeMain.getPlugin().getDiscordManager().getTextChannel("logs", true));
    ...
    @Command(name = "removevip", usage = "/<command> <player> <group>", groupToUse = Group.DIRETOR, aliases = {"removervip"})
    public void removevip(BungeeCommandArgs cmdArgs) {
        ...
        try {
            ...
            sender.sendMessage(" §c* §fO grupo §c" + args[1].toUpperCase() + "§f não existe ou não pode ser setado como temporario.");
            return;
        }
        ...
        sender.sendMessage(" §a* §fVocê removeu o grupo §a§l" + rank.name() + "§f de §a" + player.getPlayerName() + "(" + player.getUniqueId().toString().replace("-", "") + ")§f!");
