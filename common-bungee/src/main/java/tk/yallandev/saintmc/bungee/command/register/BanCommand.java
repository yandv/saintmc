...
public class BanCommand implements CommandClass {
	...
	public void banCommand(BungeeCommandArgs cmdArgs) {
		...
		Ban ban = new Ban(player.getUniqueId(), cmdArgs.isPlayer() ? cmdArgs.getPlayer().getName() : "CONSOLE", sender.getUniqueId(),
	...
	public void unbanCommand(BungeeCommandArgs cmdArgs) {
		...
		if (BungeeMain.getInstance().getPunishManager().unban(player, sender.getUniqueId(), cmdArgs.isPlayer() ? cmdArgs.getPlayer().getName() : "CONSOLE", unbanReason)) {
			sender.sendMessage(
					" §a* §fVocê desbaniu o jogador §a" + player.getPlayerName() + "§f!");
		} else {
			sender.sendMessage(" §c* §fNão foi possível banir o jogador!");
		}
	}
	...
	public void muteCommand(BungeeCommandArgs cmdArgs) {
		...
		Mute mute = new Mute(player.getUniqueId(), cmdArgs.isPlayer() ? cmdArgs.getPlayer().getName() : "CONSOLE", sender.getUniqueId(),
				args.length == 1 ? "Sem motivo" : sb.toString().trim(), -1l);
	...
	public void unmuteCommand(BungeeCommandArgs cmdArgs) {
		...
		if (args.length == 0) {
			sender.sendMessage(" §c* §fUse §a/unban <player>§f para desbanir um jogador!");
			return;
		}
		...
		if (BungeeMain.getInstance().getPunishManager().unmute(player, sender.getUniqueId(), cmdArgs.isPlayer() ? cmdArgs.getPlayer().getName() : "CONSOLE")) {
			sender.sendMessage(
					" §a* §fVocê desmutar o jogador §a" + player.getPlayerName() + "§f!");
		} else {
	...
	public void warnCommand(BungeeCommandArgs cmdArgs) {
		...
		if (BungeeMain.getInstance().getPunishManager().warn(player, warn)) {
			sender.sendMessage(
					" §a* §fVocê alertou o jogador §a" + player.getPlayerName() + "§f por §a" + warn.getReason() + "§f!");
		} else {
