...
public class GladiatorListener implements Listener {
	...
	private Map<Player, Map<Player, Challenge>> inviteMap;

	private List<Challenge> challengeList;

	private Set<Player> normalQueue;
	private Set<Player> oldQueue;

	private ActionItemStack normalItem;

	private ItemStack queueJoin = new ItemBuilder().name("§cGladiator Rápido").type(Material.INK_SACK).durability(8)
			.build();
	private ItemStack queueLeave = new ItemBuilder().name("§aGladiator Rápido").type(Material.INK_SACK).durability(10)
			.build();

	private ItemStack oldQueueJoin = new ItemBuilder().name("§cGladiator Rápido Old").type(Material.STICK).build();
	private ItemStack oldQueueLeave = new ItemBuilder().name("§aGladiator Rápido Old").type(Material.STICK).glow()
			.build();
	...
	public GladiatorListener() {
		...
		normalItem = new ActionItemStack(new ItemBuilder().type(Material.IRON_FENCE).name("§aGladiator Normal").build(),
				new Interact(InteractType.PLAYER) {
					...
					@Override
					public boolean onInteract(Player inviter, Entity entity, Block block, ItemStack item,
							ActionType action) {
						...
						if (CooldownController.getInstance().hasCooldown(player, "Gladiator Normal"))
							return false;

						if (GameGeneral.getInstance().getChallengeController().containsKey(player)
								|| GameGeneral.getInstance().getChallengeController().containsKey(inviter))
							return false;
						...
						if (inviteMap.containsKey(player)) {
							if (inviteMap.get(player).containsKey(inviter)) {
								...
								if (!challenge.hasExpired()) {
									player.sendMessage("§aO " + inviter.getName() + " aceitou o seu desafio!");
						...
						Map<Player, Challenge> map = inviteMap.computeIfAbsent(inviter, v -> new HashMap<>());

						if (map.containsKey(player)) {
							Challenge challenge = map.get(player);

							if (!challenge.hasExpired()) {
								inviter.sendMessage("§cAguarde " + DateUtils.getTime(challenge.getExpireTime())
						...
						player.sendMessage(
								"§aVocê recebeu um desafio de Gladiator Normal do " + inviter.getName() + "!");
						inviter.sendMessage("§aVocê desafiou o " + player.getName() + " para um Gladiator Normal!");
						CooldownController.getInstance().addCooldown(player,
								new ItemCooldown(player.getItemInHand(), "Gladiator Normal", 4l));
						return false;
					}
				});
	}
	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (player.getItemInHand().getType() == Material.INK_SACK) {
			...
			if (normalQueue.contains(player)) {
				...
				CooldownController.getInstance().addCooldown(player.getUniqueId(), "Gladiator Rápido", 1l);
			} else {
			...
			if (oldQueue.contains(player)) {
				player.sendMessage("§cVocê saiu da fila da Gladiator Rápido Old!");
				player.setItemInHand(oldQueueJoin);
				oldQueue.remove(player);
				CooldownController.getInstance().addCooldown(player.getUniqueId(), "Gladiator Rápido", 1l);
			} else {
				if (oldQueue.isEmpty()) {
					player.sendMessage("§aVocê entrou na fila da Gladiator Rápido Old!");
					oldQueue.add(player);
	...
	public void onGladiatorSpectator(GladiatorTrySpectatorEvent event) {
		if (challengeList.stream().filter(challenge -> challenge.isInFight(event.getPlayer())).findFirst()
				.isPresent()) {
			event.setCancelled(true);
			return;
		}

		if (challengeList.stream().filter(challenge -> challenge.getSpectatorSet().contains(event.getPlayer()))
				.findFirst().isPresent()) {
			event.setCancelled(true);
	...
	private Location handleInventory(Player player) {
		...
		player.getInventory().setItem(2, queueJoin);
		player.getInventory().setItem(4, normalItem.getItemStack());
		player.getInventory().setItem(6, oldQueueJoin);
	...
	private void handleClear(Player player) {
		...
		for (Player online : Bukkit.getOnlinePlayers()) {
			if (inviteMap.containsKey(online)) {
				Iterator<Entry<Player, Challenge>> iterator = inviteMap.get(online).entrySet().iterator();

				while (iterator.hasNext()) {
					Entry<Player, Challenge> entry = iterator.next();
