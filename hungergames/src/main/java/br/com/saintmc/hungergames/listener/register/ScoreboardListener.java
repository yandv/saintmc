				...
				new BukkitRunnable() {
					...
					public void run() {
						...
						SCOREBOARD.updateScore(player,
								new Score("Jogadores: §7" + (isPregame()
										? getGameGeneral().getPlayersInGame() + "/" + Bukkit.getMaxPlayers()
			...
			public void run() {
				...
				SCOREBOARD.updateScore(player,
						new Score(
								"Ranking: §7(" + member.getLeague().getColor() + member.getLeague().getSymbol() + "§7)",
								"ranking"));
				SCOREBOARD.updateScore(new Score("Jogadores: §7"
						+ (isPregame() ? getGameGeneral().getPlayersInGame() + "/" + Bukkit.getMaxPlayers()
				...
				if (GameMain.DOUBLEKIT)
					SCOREBOARD.updateScore(player,
							new Score("Kit 1: §e" + NameUtils.formatString(gamer.getKitName(KitType.PRIMARY)), "kit1"));
				else
					SCOREBOARD.updateScore(player,
							new Score("Kit: §e" + NameUtils.formatString(gamer.getKitName(KitType.PRIMARY)), "kit1"));
				SCOREBOARD.updateScore(player,
						new Score("Kit 2: §e" + NameUtils.formatString(gamer.getKitName(KitType.SECONDARY)), "kit2"));
			}
		}.runTaskLater(GameMain.getInstance(), 7l);
	}
	...
	public void onPlayerSelectedKit(PlayerSelectedKitEvent event) {
		...
		if (GameMain.DOUBLEKIT)
			if (event.getKitType() == KitType.PRIMARY)
				SCOREBOARD.updateScore(player,
						new Score("Kit 1: §e" + NameUtils.formatString(event.getKit().getName()), "kit1"));
			else
				SCOREBOARD.updateScore(player,
						new Score("Kit 2: §e" + NameUtils.formatString(event.getKit().getName()), "kit2"));
		else {
			if (event.getKitType() == KitType.PRIMARY)
				SCOREBOARD.updateScore(player,
