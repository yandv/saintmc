...
public class KitInfo {
	...
	public KitInfo(Player player, Kit kit, KitType kitType) {

		Gamer gamer = GameGeneral.getInstance().getGamerController().getGamer(player);
		MenuInventory menu = new MenuInventory("§7Kit " + kit.getName(), 6, true);
		...
		menu.setItem(31, new ItemBuilder().name("§fDescrição: §7" + kit.getDescription()).type(Material.NAME_TAG).build());
