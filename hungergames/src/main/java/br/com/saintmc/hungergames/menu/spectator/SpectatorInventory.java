...
public class SpectatorInventory {
	...
	public SpectatorInventory(Player player, int page) {
		...
		for (Gamer gamer : GameGeneral.getInstance().getGamerController().getGamers().stream()
			...
			items.add(new MenuItem(new ItemBuilder()
					.lore((GameMain.DOUBLEKIT
							? "\n§7Kit 1: §f" + NameUtils.formatString(gamer.getKitName(KitType.PRIMARY))
									+ "\n§7Kit 2: §f" + NameUtils.formatString(gamer.getKitName(KitType.SECONDARY))
							: "\n§7Kit: §f" + NameUtils.formatString(gamer.getKitName(KitType.PRIMARY)))
